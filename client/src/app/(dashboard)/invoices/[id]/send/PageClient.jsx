...
export default function SendInvoicePage() {
  const { id } = useParams();
  const { data, loading, error: loadError } = useFetch(() => invoicesApi.get(id), [id], { key: "invoice" });
  const inv = data?.data;
  ...
  if (loading) {
    return (
      <div className="max-w-2xl mx-auto py-16 flex items-center justify-center gap-2 text-fg-muted">
    ...
    return (
      <div className="max-w-2xl mx-auto">
        <Card className="p-10 text-center">
          ...
          <h2 className="text-xl font-bold text-fg">Invoice sent</h2>
          <p className="text-fg-muted mt-2">{inv.id} was emailed to <span className="font-semibold text-fg">{to}</span>.</p>
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <Link href={`/invoices/${inv.id}`} className="inline-flex items-center gap-2 text-sm font-medium text-fg-muted hover:text-brand mb-4"><ArrowLeft size={16} /> Back to invoice</Link>
      <PageHeader title="Send invoice" subtitle={`Email ${inv.id} to the vendor.`} />

      <Card className="p-6 sm:p-8">
        <form onSubmit={submit} className="space-y-5" noValidate>
          <div>
            <label className="block text-sm font-medium text-fg mb-1.5">To</label>
            <input value={to} onChange={(e) => setTo(e.target.value)} className={inputCls} placeholder="vendor@company.com" />
          </div>
          <div>
            <label className="block text-sm font-medium text-fg mb-1.5">Subject</label>
            <input value={subject} onChange={(e) => setSubject(e.target.value)} className={inputCls} />
          </div>
          <div>
            <label className="block text-sm font-medium text-fg mb-1.5">Message</label>
            <textarea value={message} onChange={(e) => setMessage(e.target.value)} rows={7} className={`${inputCls} resize-none`} />
          </div>

          <div className="flex items-center gap-2 text-sm text-fg-muted bg-surface-2 rounded-lg px-3.5 py-2.5">
