      ...
      {aiEnabled && showExtract && (
        <div className="rounded-xl border border-violet-200 bg-gradient-to-br from-violet-50 to-white p-4 sm:p-5 mb-5">
          <div className="flex items-center justify-between mb-2.5">
            ...
            <button onClick={() => setShowExtract(false)} className="text-slate-400 hover:text-slate-600"><X size={16} /></button>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 mt-3">
            <label className="inline-flex items-center gap-2 px-3.5 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 cursor-pointer">
              <Upload size={15} /> {image ? "Change image" : "Upload image"}
              <input type="file" accept="image/*" onChange={onPickImage} className="hidden" />
            </label>
            {image && <span className="text-xs text-slate-500 truncate max-w-[180px]">{image.name}</span>}
